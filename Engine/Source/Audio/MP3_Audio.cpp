...
namespace aio
{
	...
	void MP3_Audio::DataCallback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount)
	{
        AudioSource* source = (AudioSource*)pDevice->pUserData;
        float* out = (float*)pOutput;

        drmp3* mp3 = (drmp3*)source->decoder;
        mp3->channels;
        mp3->sampleRate;
        ...
        if (source->endPoint != 0.0f)
        {
            ...
            drmp3_uint64 endFrame = (drmp3_uint64)(source->endPoint * source->sampleRate);

            if (source->currentFrame >= endFrame) {
                framesRead = 0;
            }

            drmp3_uint64 framesUntilEnd = endFrame - (drmp3_uint64)source->currentFrame;
